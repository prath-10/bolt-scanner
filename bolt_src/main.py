...
from bolt_src.utils.coloring import COLOR, COLORED_COMBOS
from bolt_src.utils.exceptions import BoltException, HostHandlerException
from bolt_src.utils.request_handler import RequestHandler
from bolt_src.utils.logger import SystemOutLogger
from bolt_src.utils.help_utils import HelpUtilities
from bolt_src.lib.fuzzer import URLFuzzer
from bolt_src.lib.host import Host
from bolt_src.lib.scanner import Scanner, NmapScan, NmapVulnersScan, VulnersScanner
from bolt_src.lib.sub_domain import SubDomainEnumerator
from bolt_src.lib.dns_handler import DNSHandler
from bolt_src.lib.waf import WAF
from bolt_src.lib.tls import TLSHandler
from bolt_src.lib.web_app import WebApplicationScanner
...
def intro(logger):
    logger.info("""{}
                ...
                .#@@@?     #@@@5 .G@@@?        Y@@@?    P@@@P       G!      7@@@#.                   
                .&@@@J    ?@@@G:  .J#@@?      J@@G~     P@@@P     ^P@!      J@@@&.                   
                ~7PGPG5!~!JPY?^      .!YGY!^^!YP?^     ^7PGGG5!~~!?PGG!   .~!5GPPGJ~^                 
                                        .....                                                            
...
@click.option("--vulners-path", default=os.path.join(MY_PATH, "utils/misc/vulners.nse"),
              help="Path to the custom nmap_vulners.nse script."
                   "If not used, Bolt uses the built-in script it ships with.")
...
@click.option("-o", "--outdir", default="bolt_scan_results",
...
def main(target,
    ...
    try:
        ...
        if tor_routing:
            ...
            except BoltException as err:
                print("{}{}{}".format(COLOR.RED, str(err), COLOR.RESET))
                exit(3)

        main_loop = asyncio.get_event_loop()

        logger.info("{}### Bolt Scan Started ###{}\n".format(COLOR.GRAY, COLOR.RESET))
